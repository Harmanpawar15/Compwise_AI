...
# Safely convert object-type columns to category codes except specific ones
exclude_cols = ['candidate_address', 'subject_address']
for col in df.columns:
    if df[col].dtype == object:
        if col not in exclude_cols: 
            print(f"Encoding string column: {col}")
            df[col] = df[col].astype('category').cat.codes
        else:
            print(f"Leaving '{col}' as string (excluded from encoding).")
...
print("\nRanking model saved as xgb_rank_model.json")# Save the model
